...
const slidData = [
    ...
    {
        slideid: 2, content: "02 나는 LG 엘리베이터가 더 좋아.", s: "Green Technology---", p: "현대엘리베이터가 만든 새로운 길을 따라 세상은 위로 넓어집니다."
    },
    {
            ...
            <MainSlide className="main_slider" arrows={false} afterChange={(index) => setSLN(index)} ref={MAINSLISER}>
                {
                    slidData.map((slide, idx) => (
                        <figure className={`itm0${slide.slideid} ${(idx == (SLN)) && 'on'}`} key={slide.slideid}>
                            <div className="w1200">
                                <h2>
                                    {slide.content}
                                    <span>{slide.s}</span>
            ...
            <ul className="slide_dots">
                {
                    slidData.map((dots, idx) => (
                        <li className={idx == SLN ? 'on' : ''} onClick={() => (setSLN(idx), MAINSLISER.current.slickGoTo(idx))} key={dots.slideid}><span>{dots.s}</span></li>
                    ))
                }
            </ul>
